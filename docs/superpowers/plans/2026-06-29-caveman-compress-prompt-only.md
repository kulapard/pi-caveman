# Prompt-only caveman-compress Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the Python `caveman-compress` toolkit with a prompt-only Pi skill (the Pi agent compresses files itself), removing all Python from the package.
...
- The `/caveman-compress` command and the skill's compression **rules** (Preserve EXACTLY / Preserve Structure / Compress / Boundaries) are kept — only the *execution mechanism* changes from Python CLI to agent instructions.
...
- `skills/caveman-compress/SKILL.md` — rewrite the `## Process` section to prompt-only; keep the rules sections.
- `skills/caveman-compress/README.md` — rewrite to drop Python/Requires/Security; redraw "How It Work" as agent-driven.
- `skills/caveman-compress/SECURITY.md` — **delete** (only documents the Python subprocess/Snyk rating).
- `skills/caveman-compress/scripts/` — **delete** (entire Python package).
- `skills/caveman-compress/tests/` — **delete** (pytest suite).
...
- Modify: `skills/caveman-compress/SKILL.md` (replace `## Process`)
- Modify: `skills/caveman-compress/README.md` (rewrite Python-specific parts)
- Delete: `skills/caveman-compress/SECURITY.md`
...
- [ ] **Step 3: Replace the `## Process` section of `skills/caveman-compress/SKILL.md`**

In `skills/caveman-compress/SKILL.md`, replace the current `## Process` section (the block from `## Process` through the `4. Return result to user` line — i.e. the `python3 -m scripts` instructions) with:
...
- [ ] **Step 4: Rewrite `skills/caveman-compress/README.md`**
...
- [ ] **Step 5: Delete `skills/caveman-compress/SECURITY.md`**

Run: `git rm skills/caveman-compress/SECURITY.md`
...
```bash
git add skills/caveman-compress/SKILL.md skills/caveman-compress/README.md tests/compress-docs.test.mjs
git rm skills/caveman-compress/SECURITY.md
git commit -m "refactor: make caveman-compress a prompt-only skill (docs)
...
**Files:**
- Delete: `skills/caveman-compress/scripts/` (all files)
- Delete: `skills/caveman-compress/tests/` (all files)
...
Run:
```bash
git rm -r skills/caveman-compress/scripts skills/caveman-compress/tests
```
...
Run: `npm pack --dry-run 2>&1 | grep -E "caveman-compress|total files"`
Expected: only `skills/caveman-compress/README.md` and `skills/caveman-compress/SKILL.md` appear under that dir — no `scripts/`, no `tests/`, no `SECURITY.md`, no `.py`/`.pyc`.
...
git rm -r skills/caveman-compress/scripts skills/caveman-compress/tests
git commit -m "refactor: drop the Python caveman-compress toolkit and test:py wiring
...
- Produces: no shipped doc claims caveman-compress is Python / model-CLI-bound.
...
verbatim. Note `caveman-compress` is **itself Claude-bound** — it compresses via
...
Run: `npm test`
Expected: PASS — including `tests/readme.test.mjs` (still finds `pi -e`, the mode names, and `/caveman`).

- [ ] **Step 8: Commit**

```bash
git add README.md AGENTS.md
git commit -m "docs: update references for prompt-only caveman-compress
...
**Type/name consistency:** `.original.<ext>` backup naming, the `/caveman-compress` command, the forbidden-residue token list, and `files` = `["extensions","skills","agents","AGENTS.md"]` are consistent across tasks. The `compress-docs.test.mjs` rewrite (Task 1) and the doc rewrites it guards (Task 1) are in the same task, so the test and its subjects land together.

**Cross-task ordering note:** Task 1's test forbids Python residue only in `skills/caveman-compress/{SKILL,README}.md` (rewritten in Task 1). The root README / AGENTS.md Python references are cleaned in Task 3 and are not covered by Task 1's test — Task 3 Step 6 grep is their gate. No task leaves the suite red.
